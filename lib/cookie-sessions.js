...
(function() {
  ...
  exports = module.exports = function(settings) {
    ...
    return function(req, res, next) {
      var _writeHead,
        _this = this;
      if (0 !== url.parse(req.url).pathname.indexOf(s.path)) {
        return next();
      }
      req.session = exports.readSession(s.session_key, s.secret, s.timeout, req);
      if (!req.session && s.emptySession) {
        req.session = {};
      }
      ...
      res.writeHead = function(statusCode) {
        ...
        if (cookiestr) {
          if (s.useExpires) {
            cookiestr += '; expires=' + exports.expires(s.timeout);
          }
          if (s.useMaxAge) {
            cookiestr += '; max-age=' + (s.timeout / 1000);
          }
          if (s.path) {
            cookiestr += '; path=' + s.path;
          }
          if (s.domain) {
            cookiestr += '; domain=' + s.domain;
          }
          if (s.secure) {
            cookiestr += '; secure';
          }
